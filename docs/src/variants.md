...
| **TDT** | [`tdt_loss_batched`](@ref) | `logits` `(V, T, U+1, B)` + `dur_logits` `(D, T, U+1, B)` + `durations::Vector{Int}` | Frame-skipping decode, Parakeet-TDT-style models, multi-blank / variable stride | Generalizes RNN-T with a duration head (Xu et al., ICML 2023) |
