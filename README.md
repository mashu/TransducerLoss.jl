...
Batched **transducer-loss family** for Julia — GPU-capable via KernelAbstractions,
...
grad = Zygote.gradient(l -> rnnt_loss_batched(l, targets, input_lengths), logits)[1]
```
...
|---|---|---|
| RNN-T (Graves 2012) | `rnnt_loss_batched` | baseline transducer |
| TDT (Xu et al., ICML 2023) | `tdt_loss_batched` | duration head, frame-skipping decode |
| Monotonic RNN-T / RNA | `monotonic_rnnt_loss_batched` | TDT special case `durations = [1]` |
